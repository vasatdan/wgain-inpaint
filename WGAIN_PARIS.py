...
# Warning - the dataset in this repository
path = "./datasets/paris/paris128_%s"
paris_train = data.Dataset.list_files(path%'train' + str('/*.jpg'))
paris_train_d = prepare_training_dataset(paris_train, batch_size, side)
...
for epoch in range(1,2001):
    ...
    losses_m = []
    losses_d = []
    for i, (origX, newX, mask, randZ) in enumerate(paris_train_d):
    ...
    generator.save_weights("saved_models_paris/gultimate_latest")
    critic.save_weights("saved_models_paris/cultimate_latest")
    if epoch%10==0:
        generator.save_weights("saved_models_paris/gultimate_ckpt%d" % epoch)
        critic.save_weights("saved_models_paris/cultimate_ckpt%d" % epoch)


# Final save of weights
generator.save_weights("saved_models_paris/gultimate_final")
critic.save_weights("saved_models_paris/cultimate_final")
